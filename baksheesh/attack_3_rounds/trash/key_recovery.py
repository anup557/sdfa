...
def attack_r1(trail_list, r1_keyspace, cip):    
    ...
    #print('last_layer_diff_list::\n', last_layer_diff_list)
    #print('trail list::\n', trail_list[4], trail_list[5])
...
def attack_r2(trail_list, r1_keyspace, cip_fcip_list):    
    ...
    r2_keyspace = [list(product(*[r1_keyspace[i + 8*j] for j in range(4)])) for i in range(8)]
    r2_keyspace01 = list(product(r2_keyspace[0], r2_keyspace[1]))
    print('prev len: ', len(r2_keyspace01))

    # checking for these nibbles only
    for nibble_idx in [0, 1, 2, 3]:
        for cip_fcip_idx, cip_fcip in enumerate(cip_fcip_list):
            if (trail_list[cip_fcip_idx][1][0][nibble_idx] != 0):
                ...
                count = 0
                for key_idx, key4 in enumerate(r2_keyspace01):
                    if(r2_keyspace01[key_idx] == 9999):
                        continue
                    ...
                    for j in range(4):
                        last_key[8*j] = key4[0][j]
                        last_key[1 + 8*j] = key4[1][j]

                    original_last_key = round_key_list[NO_OF_ROUNDS].copy()
                    # last_key = original_last_key.copy()
                    # for i in [0, 8, 16, 24, 1, 9, 17, 25]:
                    #     last_key[i] = inv_perm(original_last_key)[i]
                    ...
                    # ------------------------------------------------------------------------------
                    dec_cip = [dec_cip^scnd_last_key for dec_cip, scnd_last_key in zip(dec_cip, scnd_last_key)]
                    dec_fcip = [dec_fcip^scnd_last_key for dec_fcip, scnd_last_key in zip(dec_fcip, scnd_last_key)]
                    ...
                    in_diff = [dec_cip^dec_fcip for dec_cip, dec_fcip in zip(dec_cip, dec_fcip)]

                    # checking whether the input diff is same as the diff in trail or not
                    if(in_diff[nibble_idx] != trail_list[cip_fcip_idx][1][0][nibble_idx]):
                        print('\n\nnibble idx: \t', nibble_idx)
                        print('in diff: \t', in_diff)
                        print('trail list: \t', trail_list[cip_fcip_idx])

                        r2_keyspace01[key_idx] = 9999

                        return r2_keyspace
    for i in r2_keyspace01:
        print(i)
    return r2_keyspace
