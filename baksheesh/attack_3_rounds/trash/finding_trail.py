# this is for default cipher


# gist: calculate the trails for 3 rounds for simple key schedule with random msg, nibble val and nibble idx for 100
# times and each of the time it gives the correct trail. Keep the file in "/home/anup/Desktop/github/dfa_on_default/oracle" 
# folder to execute.
# ---------------------------------------------------------------------------------------------------------------
...
def main():
    ...
    state_list = [[] for i in range(NO_OF_ROUNDS+1)]
    ...
    fault_state_list = [[] for i in range(NO_OF_ROUNDS+1)]
    ...
    # ---------------------------------------------------------------------------------------------------
    cip = state_list[NO_OF_ROUNDS].copy()
    fcip = fault_state_list[NO_OF_ROUNDS].copy()
    ...
    for i in range(3):
        if (trail_list[0][i][0] == original_last_3_trail[i]):
            success = success + 1
    if (success == 3):
        print('hola.')
        return 1
