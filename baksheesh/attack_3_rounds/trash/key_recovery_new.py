...
def attack_r2(trail_list, r1_keyspace, cip_fcip_list):    
        ...
        for cip_fcip_idx, cip_fcip in enumerate(cip_fcip_list):
            if (trail_list[cip_fcip_idx][1][0][nibble_idx] != 0):
                ...
                for key_idx, key4 in enumerate(r2_keyspace):
                    ...
                    if((in_diff0[nibble_idx] != trail_list[cip_fcip_idx][1][0][nibble_idx]) and 
                       (in_diff1[nibble_idx] != trail_list[cip_fcip_idx][1][0][nibble_idx])):
    ...
    print('*****************************************************************************************')
    r2_keyspace = list(product(r1_keyspace[1], r1_keyspace[3], r1_keyspace[4], r1_keyspace[6],
                                 r1_keyspace[9], r1_keyspace[11], r1_keyspace[12], r1_keyspace[14],
                                 r1_keyspace[17], r1_keyspace[19], r1_keyspace[20], r1_keyspace[22],
                                 r1_keyspace[25], r1_keyspace[27], r1_keyspace[28], r1_keyspace[30]))
        ...
        for cip_fcip_idx, cip_fcip in enumerate(cip_fcip_list):
            if (trail_list[cip_fcip_idx][1][0][nibble_idx] != 0):
                ...
                for key_idx, key4 in enumerate(r2_keyspace):
                    ...
                    for idx_j, j in enumerate([1, 3, 4, 6, 9, 11, 12, 14, 17, 19, 20, 22, 25, 27, 28, 30]):
                        last_key[j] = key4[idx_j]

                    # eq_nibble_list contains eles like [nibble_pos, nibble_val]
                    for ele in eq_nibble_list:
                        last_key[ele[0]] = ele[1]
    ...
    for key4 in r2_keyspace:
        ...
        # storing the nibble values that varries for the last key
        for idx_j, j in enumerate([1, 3, 4, 6, 9, 11, 12, 14, 17, 19, 20, 22, 25, 27, 28, 30]):
            last_key[j] = key4[idx_j]

        # storing the fixed nibble values in the last key
        for ele in eq_nibble_list:
            last_key[ele[0]] = ele[1]
