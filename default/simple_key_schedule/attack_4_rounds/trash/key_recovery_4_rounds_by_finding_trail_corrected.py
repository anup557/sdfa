            ...
            for cip_fcip_idx, cip_fcip in enumerate(cip_fcip_list):
                if (trail_list[cip_fcip_idx][2][0][nibble_idx] != 0):
                    ...
                    for key_idx, key4 in enumerate(r2_keyspace[group_idx]):
                        ...
                        # if (group_idx == 4):
                            # out_diff =(dec_cip[nibble_idx] ^ dec_fcip[nibble_idx])
                            # print('nibble idx',nibble_idx, ': ', 'out diff: ', out_diff)
...
def attack_r3(trail_list, r1_keyspace, r2_keyspace, cip_fcip_list):    
    ...
    for group_idx_last in [0, 1]:
        ...
        # # to check whether left half and right half keys are present or not
        ...
        #     # checking whether the input diff is same as the diff in trail or not
        #     if (last_key == [8, 0, 8, 0, 9, 0, 9, 0, 10, 0, 10, 0, 11, 0, 11, 0, 12, 0, 12, 0, 13, 0, 13, 0, 14, 0, 14, 0, 15, 0, 15, 0]):
        #         print('original key for left half is here.')

        #     if (last_key == [0, 0, 0, 0, 0, 1, 0, 1, 0, 2, 0, 2, 0, 3, 0, 3, 0, 4, 0, 4, 0, 5, 0, 5, 0, 6, 0, 6, 0, 7, 0, 7]):
        #         print('original key for right half is here.')
        # continue
            ...
            for cip_fcip_idx, cip_fcip in enumerate(cip_fcip_list):
                ...
                if (trail_list[cip_fcip_idx][1][0][nibble_idx] != 0):
                    ...
                    for key_idx, key in enumerate(dummy_r3):
                        ...
                        # print('r2 keyspace: ', r2_keyspace[nibble_idx%8])
                        # print('\nr2 keyspace: ', r2_keyspace[nibble_idx%8][nibble_idx//8])
                        ...
                        if(in_diff == trail_list[cip_fcip_idx][1][0][nibble_idx]):
                            ...
                            # for the left half, only take those keys that passes nibble idx 15 and for the right, take those that passes nibble 31
                            #if (nibble_idx == 15):
                                #r3_keyspace[group_idx_last].append(key)
                                
                            #r3_keyspace[group_idx_last].append(key)
                            #print("r3_keyspace::", r3_keyspace[group_idx_last])
...
def attack_r4(trail_list, r3_keyspace, r2_keyspace, r1_keyspace, cip_fcip_list):    
    ...
    # # to check whether the original key is there or not
