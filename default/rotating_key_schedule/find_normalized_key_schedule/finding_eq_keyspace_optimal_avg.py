...
# some faults are given in some of the nibbles. In the second call other faults are given and in most of the cases
# the equivalent keyspace of 4 rounds are calculated. The results are given in the next part. This is code to
# calculate the avg times when the prog finds equivalent key space successfully.

# following are the results in percentage for the number of faults in some specific positions
#     for 32 + 0 faults: 100
#     for 16 + 12 faults: 78
#     for 16 + 8 faults: 75
#     for 16 + 4 faults: 39
...
def attack_dec(original_known_keys, starting_keyset):
    ...
    for nibble_idx in range(32):
        ...
        # nibble
        diff_list = get_delta_dec(known_keys, len(known_keys) + 1, nibble_idx)
...
def get_delta_dec(known_keys, round_nr: int, idx: int):
    # initializing the last and the second last diff list to store the corresponding diff
    diff_list = []

    # in the first call number there is only one trail list, so just use the diffs of that list only
    if (call_number == 0):
    ...
    # in the second call number there are two trail lists, for this take the diffs from both of them
    if (call_number == 1):
        # --------------------------------------------------------
        # for the first trail list
        # --------------------------------------------------------
        for trail_idx, trail in enumerate(trail_list):
            # if there is no difference in that nibble
            if (trail[3-round_nr][0][idx] == 0):
                continue
            ...
            # ---------------------------------
            # inversing two rounds
            # ---------------------------------
            # applying the first round inverse 
            # adding round key
            ...
            # applying the second round inverse 
            # adding round key
            ...
            a, b, in_diff = cip[idx], fcip[idx], trail[3-round_nr][0][idx]

            # appending the corresponding nibble values of cip, faulty cip and input diff
            diff_list.append([a, b, in_diff])

        # --------------------------------------------------------
        # for the second trail list
        # --------------------------------------------------------
        for trail_idx, trail in enumerate(trail_list_1):
            # if there is no difference in that nibble
            if (trail[5-round_nr][0][idx] == 0):
                continue

            # extracting the corresp cip and fcip of the corresp trail
            cip = inv_perm(cip_fcip_list_1[trail_idx][0])
            fcip = inv_perm(cip_fcip_list_1[trail_idx][1])
...
def single_key(keyspace):
    ...
    for nibble in keyspace:
        # if in each of the nibble more than one eq key is there, then the flag is off i.e. 0
        if len(nibble) != 1:
            flag = 0
...
def call_oracle(base_key, round_key, call_number):
    ...
    # in the first call give fault in all the nibbles
    if (call_number == 0):
        fix_fault_nibble = [2*i for i in range(16)]
        # fix_fault_nibble = [i for i in range(32)]
        # fix_fault_nibble = [4*i for i in range(8)]

    if (call_number == 1):
        # in the second call give fault in the following nibbles
        fix_fault_nibble = []
        # fix_fault_nibble = [4*i for i in range(8)]
        # fix_fault_nibble = [0, 2, 4, 6, 8, 12, 16, 18, 20, 24, 28, 30]
        # fix_fault_nibble = [2*i for i in range(16)]
...
def main():
    ...
    global trail_list, cip_fcip_list
    trail_list, cip_fcip_list = call_oracle(base_key, round_key, call_number)

    # recovering equivalent key space
    global k3
    k3, flag = single_key(attack_dec(keys, range(4)))
    # if the flag is 1, then the exp fails, so return 0
    if (flag == 0):
        return 0
    keys.append(k3)
    ...
    global k2
    k2, flag = single_key(attack_dec(keys, range(4)))
    # if the flag is 1, then the exp fails, so return 0
    if (flag == 0):
        return 0
    keys.append(k2)
    ...
    # --------------------------------------------------------
    call_number = 1
    global trail_list_1, cip_fcip_list_1
    trail_list_1, cip_fcip_list_1 = call_oracle(base_key, round_key, call_number)

    # give the call number as anothrer condition for the round number
    k1, flag = single_key(attack_dec(keys, range(4)))
    # if the flag is 1, then the exp fails, so return 0
    if (flag == 0):
        return 0
    keys.append(k1)
    ...
    for nibble_idx in range(32):
        if (inv_perm(nk[0])[nibble_idx] not in k0[nibble_idx]):
            flag = 0
            break
    if (flag == 0):
        return 0
    ...
    # if all the assertations are passed then return its success
    return 1
...
if __name__ == '__main__':
    ...
    for i in range(no_of_times):
        ...
        print('\n')
        print('*'*80)
        print('for times ' + str(i) + ': ')
        print('*'*80)

        # if the main function returns true then increase the count
        if (main() == 1):
            cnt = cnt+1

    print('\navg case of success: ', str((cnt*100)/no_of_times))
