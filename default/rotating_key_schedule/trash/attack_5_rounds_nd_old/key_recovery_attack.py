# program desc:
# -----------------------------------------------------------------------------------------------------
# This is key recovery attack for rotating key schedule according to our work for 5 rounds. In this prog 
# initially we give faults at round 20 (for 8 rounds). Then reduce the keyspace of k3, k2, k1 in equivalent
# keyspaces then use the trail of 5 rounds to recover the keyspace of k0. At first decrypt the rounds 27,
# 26, 25 by k3, k2, k1 resp and then use our attack procedure  to reduce the keyspace of k0. By this
# procedure after giving 
# 4 faults reduced size: 2.7164278438301173 
# 5 faults reduced size: 0.0 
...
def finding_trail_3_round(cip_diff, delta, idx, trail_list):
    ...
    dummy_frst_output = inv_permute_bits(mid_layer_input)
...
def finding_trail_5_round(last_diff, fault_val, fault_nibble, trail_list):
    ...
    last_layer_output = inv_permute_bits(last_diff)
    ...
    for qr in range(8):
        ...
        for trail in all_poss_last_layer_input:
            ...
            inter_diff = inv_permute_bits(inter_diff)
    ...
    for i in range(len(fourth_round_trail)):
        ...
        trail_list = finding_trail_3_round(fourth_round_trail[i], fault_val, fault_nibble, trail_list)
        ...
        full_trail_list.append([fourth_round_trail[i], inv_permute_bits(fifth_round_trail[i])])
        full_trail_list.append([fifth_round_trail[i], inv_permute_bits(last_diff)])

    return full_trail_list
        ...
        for cip_fcip_idx, cip_fcip in enumerate(cip_fcip_list):
           ...
           # if the diff appears in the nibble idx, then do the following 
            if (trail_list[cip_fcip_idx][0][0][nibble_idx] != 0):
                ...
                # initializing the count for each key of r5_keyspace
                count = 0
                rej_count = 0
...
def main(number_of_faults ):
    ...
    for fault_idx, fault in enumerate(fault_list):
        ...
        new_trail_list = finding_trail_5_round(cip_diff, fault, idx, trail_list)
    ...
    # print('\nr4 keyspace:', r4_keyspace)

    r5_keyspace = attack_r5(trail_list, r4_keyspace, k0, k1, k2, k3, cip_fcip_list)
    # print('\nr5 keyspace:', r5_keyspace)

    # storing the reduced keyspace size after r4 attack
    reduced_keyspace_size = math.log2(len(r5_keyspace))

    if (nks_0_key in r5_keyspace):
...
if __name__=='__main__':
    ...
    print('For 5th round attack (using rotating key schedule):')
    print('*'*100)
